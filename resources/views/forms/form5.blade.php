<x-base-layout>
    ...
    <div class="text-center">
        <p class="text-2xl font-bold mb-4">Select a file</p>
        <div class="mb-2 bg-black border border-white rounded w-64 px-2 text-left">
            <input class=" hidden" type="file" id="nameInput" name="human" onchange="enableSubmit()" />
            <label id="label" for="nameInput">Choose files</label>
        </div>
    ...
    <script>
        function enableSubmit() {
            ...
            const nameInput = document.getElementById('label').innerHTML = 'File Uploaded !';
        ...
        document.getElementById('submitButton').addEventListener('click', function(event) {
            if (this.classList.contains('button-disabled')) {
                ...
                window.location.href = "{{ route('form6') }}"; // Redirect if not disabled
            }
        });
    </script>
    </body>
